sap.ui.core.mvc.Controller.extend("catalog.view.Detail", {
  ...
  onRoutePatternMatched: function(oEvent) {
    ...
    this.oController = this.getView().getController();

    // if (!this.oDialog) {
    //  this.oDialog = sap.ui.xmlfragment("view.PopUp", this.oController);
    // }
  ...
  onRouteMatched: function(oEvent) {
    ...
    window.oObject = this.getView().getBindingContext().getObject();
    window.oController = this.getView().getController();
  ...
  performInitialization: function(oEvent) {
    ...
    this[window.oObject.UISettings.Tab1Hand]();
    //Initialize tab
  ...
  attachHandlers: function(path, mControl) {
    ...
    for (iter = 1; iter < 5; iter++) {
      if (this.getView().getBindingContext().getObject(path + iter) === true) {
        this.getView().byId(mControl + iter).attachPress(this[this.getView().getBindingContext().getObject(path + iter + "Hand")], this.getView()
          .getController());
      }
    }
  },
  ...
  onInputChange: function(oEvent) {
    if (oEvent.getParameters().value < window.oObject.Fcat.UnitMin) {
      var message = this.getView().getModel("i18n").getResourceBundle().getText("unitMin", [window.oObject.Fcat.UnitMin]);
      sap.ui.getCore().byId(oEvent.getSource().getId().substring(0, 10) + "idInput").setValue(window.oObject.Fcat.UnitMin);
      sap.m.MessageToast.show(message, {
        duration: 4000
      });
    } else if (oEvent.getParameters().value > window.oObject.Fcat.UnitMax) {
      var message = this.getView().getModel("i18n").getResourceBundle().getText("unitMax", [window.oObject.Fcat.UnitMax]);
      sap.ui.getCore().byId(oEvent.getSource().getId().substring(0, 10) + "idInput").setValue(window.oObject.Fcat.UnitMax);
      sap.m.MessageToast.show(message, {
        duration: 4000
      });
    } else {
  ...
  getStepValue: function(oEvent) {
    //Max - Min / 10
    return ((window.oObject.Fcat.UnitMax - window.oObject.Fcat.UnitMin) / 10);
  },
  ...
  onTest: function(oEvent) {
    alert(window.oObject.UISettings.Tab1);
  },

  onAddBenefit: function(oEvent) {
    jQuery.sap.require("sap.m.MessageBox");
    sap.m.MessageBox.show(
    	"Do you want to add this benefit to your benefits?", {
        icon: sap.m.MessageBox.Icon.WARNING, //cart-full
        title: "Add benefit",
        actions: [sap.m.MessageBox.Action.OK, sap.m.MessageBox.Action.CANCEL],
        onClose: this.getView().getController().fnCallbackMessageBox,
        styleClass: ""
      }
    );
  },

  fnCallbackReturn: function(oAction) {

  },

  fnCallbackError: function(oAction) {

  },

  fnCallbackMessageBox: function(oAction) {
    if (oAction == "OK") {
      var oFuncParams = {
        Pernr: window.oObject.Pernr,
        Barea: window.oObject.Barea,
        Bplan: window.oObject.Bplan,
        FrontendItem: window.oObject.FrontendItem
      };

      window.oModel.callFunction('addBenefit', 'POST', oFuncParams,
        null, handleReturn, handleError);

      var handleReturn = function expression() {
        alert("Return Jaime");
      };
      var handleError = function expression() {
        alert("Error Jaime");
      };
